...
trip_duration_by_weekday <- read_csv("result/trip-duration-by-weekday.csv")
...
trip_duration_by_weekday <- trip_duration_by_weekday %>%
    mutate(weekday = factor(weekday, levels = c("Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday")))
...
trip_duration_by_weekday_plot <- ggplot(
    trip_duration_by_weekday,
    mapping = aes(fill = member_casual, x = weekday, y = duration)
) +
    geom_bar(position = "dodge", stat = "identity") +
    labs(title = "Median trip duration per weekday by type of customer", x = "Weekday", y = "Duration (min)")
...
ggsave("plots/trip-duration-by-weekday.png", trip_duration_by_weekday_plot, width = 10, height = 5)
